MLPMixer in a nutshell. Recall that dot-product in MSAttn is (Q . K^T) = (x . W_Q^T) . (x . W_K^T)^T: Using MSAttn as analogy and ignore activation function, <br> then each MLPMixer block that contains a pair of (token-mixer, channel-mixer) would be (x^T . W_K1^T . W_K2^T)^T . W_Q1^T . W_Q2^T
