...
- [ ] Add more Conv block at beginning of every depth and more blocks at early depth following Alter-ResNet design in [How Do Vision Transformers Work?](https://openreview.net/forum?id=D78Go4hVcxO): Let's start ConvNeXt block first since it's easy to tweak MLP-Mixer block to get ConvNeXt; then ResNeXt or ResNet blocks as long as either of them have DW-Separable conv design from Efficient. Now, because an exact 3x3 convolution in numpy will be very slow, I will need to learn how to unroll input tensor so that I can apply convolution in the manner of dot-product matrices, in other words I need to reverse-engineer how torch.nn.functional.Conv1d works. This makes this task alone 2nd hardest. Once I manage to include Conv block, then follow by Alter-ResNet design I will need to remove patchify. 
